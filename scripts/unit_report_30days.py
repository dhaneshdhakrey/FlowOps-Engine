...
logger = start_logging("unit_report_30days.log")


def process_unit_report_30days(target_date: str):
    """
    Fetch and save Unit Report 30days data for the given date.
    Returns standardized dict format.
    """
    try:
        logger.info(f"Unit Report 30days ETL started for {target_date}")

        # Paths
        folder_path = f"/home/hadoop/dhanesh/au_360/au_analytics/unit_report_30days/{target_date}"
        os.makedirs(folder_path, exist_ok=True)
        file_path = os.path.join(folder_path, f"response_{target_date}.json")

        # Hive query
        query = f"SELECT * FROM au_360.unit_report_30days WHERE report_date='{target_date}'"
        ...
        logger.error(
            f"Unit Report 30days ETL failed for {target_date}: {str(e)}", exc_info=True
        )
        return {
            "status": "error",
            "total_rows": 0,
...
if __name__ == "__main__":
    ...
    ans = process_unit_report_30days(date_arg) 
    print(ans)

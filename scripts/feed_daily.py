...
logger = start_logging("feed_daily_job.log")

def process_feed_daily(execution_date: str = None):
    """
    Fetch daily feed report data from Hive and save as JSON.
    Handles no-data overwrite rules.
    Returns standardized dict format.
    """
    try:
        # pick Airflow execution_date if given, otherwise today
        date = execution_date or datetime.now().strftime("%Y-%m-%d")
        logger.info(f"Feed Daily ETL started for {date}")

        # output folder & file
        folder_path = f"/home/hadoop/dhanesh/au_360/au_analytics/feeds/feed_daily/{date}"
        ...
        logger.info(f"Feed Daily ETL finished for {date}")
        ...
        logger.error(f"Feed Daily ETL failed: {str(e)}", exc_info=True)
        return {
            "status": "error",
            "total_rows": 0,
            "total_columns": 0,
...
if __name__ == "__main__":
    ...
    ans = process_feed_daily(date_arg) 
    print(ans)

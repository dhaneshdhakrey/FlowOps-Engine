...
logger = start_logging("unit_wise_report_7days.log")

def process_unit_wise_report_7days(cur_date):
    # cur_date = datetime.now().strftime("%Y-%m-%d")
    folder_path = f"/home/hadoop/dhanesh/au_360/au_analytics/unit_wise_report_7days/{cur_date}"
    os.makedirs(folder_path, exist_ok=True)

    file_path = os.path.join(folder_path, f"response_{cur_date}.json")
    query = f"SELECT * FROM au_360.unit_vise_report_7days WHERE report_date='{cur_date}'"
    logger.info(f"Executing query for {cur_date}")
    ...
    try:
        ...
        logger.info(f"saving date in path::")
        return {
            "status": "success",
...
if __name__ == "__main__":
    ...
    ans = process_unit_wise_report_7days(date_arg) 
    print(ans)
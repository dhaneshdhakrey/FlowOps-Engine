...
logger = start_logging("feedFeedVersion_hourly_job.log")

def process_feed_feedversion_hourly(execution_date: str = None):
    """
    Fetch feed version hourly report from Hive and save as JSON.
    ...
    try:
        ...
        logger.info(f"FeedFeedVersion Hourly ETL started for {date}")

        # Folder & file path
        folder_path = f"/home/hadoop/dhanesh/au_360/au_analytics/feeds/feedFeedVersion_hourly/{date}"
        ...
        query = f"""
            SELECT * 
            FROM au_360.auw_feed_version_hourly_report
            WHERE feed_hourly_date = '{date}'
        """
        logger.info(f"Executing query for {date}")
        ...
        logger.info(f"FeedFeedVersion Hourly ETL finished for {date}")
        ...
        logger.error(f"FeedFeedVersion Hourly ETL failed: {str(e)}", exc_info=True)
        return {
            "status": "error",
            "total_rows": 0,
            "total_columns": 0,
...
if __name__ == "__main__":
    ...
    ans = process_feed_feedversion_hourly(date_arg) 
    print(ans)

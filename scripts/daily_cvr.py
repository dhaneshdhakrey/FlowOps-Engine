...
logger = start_logging("daily_cvr_job.log")


def fetch_daily_cvr(target_date: str):
    """
    Fetch daily_category_vise_report data for a given date.
    Returns standardized dict format.
    """
    try:
        logger.info(f"Daily CVR ETL started for {target_date}")

        # Paths
        folder_path = f"/home/hadoop/dhanesh/au_360/au_analytics/daily_cvr/{target_date}"
        os.makedirs(folder_path, exist_ok=True)
        file_path = os.path.join(folder_path, f"response_{target_date}.json")

        # Query
        query = f"""
            SELECT parentcategory, stories, users, views
            FROM au_360.daily_category_vise_report
        ...
        column_names = ["parentcategory", "stories", "users", "views"]
        ...
        logger.error(f"Daily CVR ETL failed for {target_date}: {str(e)}", exc_info=True)
        return {
            "status": "error",
            "total_rows": 0,
            "total_columns": 0,
...
def process_daily_cvr(cur_date):
    """
    ...
    current_day = fetch_daily_cvr(cur_date)
    previous_day = fetch_daily_cvr(prev_date)
...
if __name__ == "__main__":
    ...
    ans = process_daily_cvr(date_arg) 
    print(ans)

...
logger = start_logging("unit_wise-report_30days.log")

def process_unit_wise_report_30days(cur_date):
    # cur_date = datetime.now().strftime("%Y-%m-%d")
    folder_path = f"/home/hadoop/dhanesh/au_360/au_analytics/unit_wise_report_30days/{cur_date}"
    os.makedirs(folder_path, exist_ok=True)
    logger.info(f"Processing unit_wise_report_30days for date: {cur_date}") 
    file_path = os.path.join(folder_path, f"response_{cur_date}.json")
    query = f"SELECT * FROM au_360.unit_vise_report_30days WHERE report_date='{cur_date}'"
...
if __name__ == "__main__":
    ...
    ans = process_unit_wise_report_30days(date_arg) 
    print(ans)

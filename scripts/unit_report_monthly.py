...
logger = start_logging("unit_report_monthly.log")


def process_unit_report_monthly(report_month: str):
    """
    Fetch and save Unit Report Monthly for a given month.
    Returns standardized dict with status, rows, cols, docs.
    """
    try:
        logger.info(f"Unit Report Monthly ETL started for {report_month}")

        folder_path = f"/home/hadoop/dhanesh/au_360/au_analytics/unit_report_monthly/{report_month}"
        os.makedirs(folder_path, exist_ok=True)
        file_path = os.path.join(folder_path, f"response_{report_month}.json")

        query = f"SELECT * FROM au_360.unit_report_monthly WHERE report_month='{report_month}'"
        ...
        logger.error(
            f"Unit Report Monthly ETL failed for {report_month}: {str(e)}",
            exc_info=True,
        )
        return {
            "status": "error",
...
if __name__ == "__main__":
    ...
    ans = process_unit_report_monthly(date_arg) 
    print(ans)